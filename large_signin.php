...
if($stmt = $con->prepare('SELECT id, password FROM large_users   WHERE email = ?')){
    ...
    if($stmt->num_rows>0){
        ...
        if($stmt = $con->prepare('INSERT INTO large_users (name, Phone, email, password, town, district, state) VALUES (?, ?, ?, ?, ?, ?, ?)')){
