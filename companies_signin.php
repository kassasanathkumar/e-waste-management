...
if($stmt = $con->prepare('SELECT id, password FROM recycle_companies  WHERE email = ?')){
    ...
    if($stmt->num_rows>0){
        ...
        if($stmt = $con->prepare('INSERT INTO recycle_companies (name, Phone, email, password, town, district, state) VALUES (?, ?, ?, ?, ?, ?, ?)')){
            ...
            echo "<script>setTimeout(function() {window.location.href = './companies_home_page.php';}, 3000);</script>";
        }
        else{
            echo 'ERRor occured!';
        }
